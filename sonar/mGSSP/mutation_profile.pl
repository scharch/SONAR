...
if(@ARGV<1||@ARGV%2>0){die "
	usage: script 
	-f sequence file processed from SONAR pipeline, with one reperesentative sequence per lineage, only Fasta format accepted. The sequence head need to have v gene annotation, such as V_Gene=IGHV1-2*02
...
Example:
mutation_profile.pl -f test.fa 

Created by Zizhang Sheng 2017-02-23.
