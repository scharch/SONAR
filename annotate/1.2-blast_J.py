...
def main():
	...
	if arguments['--cluster']:
		...
		if not arguments['--noC']:
			...
			pbs.close()
			cjobid = subprocess.check_output( [qsub, "--parsable", "--array=[1-%d]"%f_ind, "%s/cblast.sh"%prj_tree.jgene] )
			...
			monitor.write( PBS_STRING%("cMonitor-%s"%prj_name, "2G", "0:30:00", "#SBATCH --dependency=afterany:%s\n%s >> %s/qmonitor.log 2>&1"%(cjobid.decode(), check, prj_tree.logs)))
			monitor.close()
			comintorid = subprocess.check_output( [qsub, "--parsable", "%s/cmonitor.sh"%prj_tree.jgene] )
		...
		if not arguments['--noD']:
			...
			pbs.close()
			djobid = subprocess.check_output( [qsub, "--parsable", "--array=[1-%d]"%f_ind, "%s/dblast.sh"%prj_tree.jgene] )
			...
			monitor.write( PBS_STRING%("dMonitor-%s"%prj_name, "2G", "2:00:00", "#SBATCH --dependency=afterany:%s\n%s >> %s/qmonitor.log 2>&1"%(djobid.decode(), check, prj_tree.logs)))
			monitor.close()
			dmonitorid = subprocess.check_output( [qsub, "--parsable", "%s/dmonitor.sh"%prj_tree.jgene] )
		...
		pbs.close()
		jjobid = subprocess.check_output([ qsub, "--parsable", "--array=[1-%d]"%f_ind, "%s/jblast.sh"%prj_tree.jgene] )
		...
		monitor.write( PBS_STRING%("jMonitor-%s"%prj_name, "2G", "12:00:00", "#SBATCH --dependency=afterany:%s\n%s >> %s/qmonitor.log 2>&1"%(":".join([x for x in [jjobid.decode().strip(), comintorid.decode().strip(), dmonitorid.decode().strip()] if not x==""]), check, prj_tree.logs))) #wait for C and D to finish before calling 1.3 (if relevant)
		monitor.close()
