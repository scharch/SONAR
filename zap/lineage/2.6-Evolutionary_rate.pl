...
if(@ARGV%2>0||@ARGV==0){die "Usage: Evolutionary_rate.pl evolutionary rate calculation
	-i input antibody nucleotide VDJ sequence file with fasta format
	-CDR sequences of the three CDRs
	-FW sequences of the four FWs
	-spliter which marker used to seperate seq name, default:_
	-nc which column is time info after parsing using \'_\' in the sequence name, start from 1. default:2
	-pop model of coalecent population size, constant or bayesian_skyline, default is later
	-o output file prefix
	-n how many sequences to choose from each time point. default is 25
	-chainLength number of steps to run, default:10000000
	-storeEvery store results every n steps, default:5000
	-log store a tree every n step, default:1000
	-beast path to beast2 program
	";}
	
my %para=@ARGV;
if(!$para{'-i'}){die "No input sequence file\n";}
if(!$para{'-beast'}){die "Please set up absolute path to beast program\n";}
if(!$para{'-n'}){$para{'-n'}=25;}
if(!$para{'-nc'}){$para{'-nc'}=2;}
if(!$para{'-o'}){$para{'-o'}=$para{'-i'};$para{'-o'}=~s/\..*//;}
...
my $seq_CDR='';
my $seq_FW='';
my ($seq,$seq_name_selected)=&read_fasta($para{'-i'},$para{'-n'});#read sequences and select n sequences for each time point
